...
import { MapPin, Calendar, BookOpen, CheckCircle, Clock, AlertCircle, Edit3, Star, Warehouse, RefreshCw } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { userAPI, reservationsAPI, stallsAPI } from "../../api/axios";
...
const computeStats = (reservations: Reservation[], availableStalls: number): DashboardStats => {
  const activeReservations = reservations.filter((reservation) => reservation.status === "ACTIVE");
  const completedSetups = activeReservations.filter((reservation) => reservation.literaryGenres?.length).length;
  const pendingActions = Math.max(activeReservations.length - completedSetups, 0);

  return {
    totalReservations: reservations.length,
    activeReservations: activeReservations.length,
    availableStalls,
    maxStallsAllowed: MAX_STALLS,
    pendingActions,
    completedSetups,
  };
};
...
const sortReservations = (list: Reservation[]) =>
  [...list].sort(
    (a, b) =>
      new Date(b.reservationDate || b.createdAt).getTime() - new Date(a.reservationDate || a.createdAt).getTime()
  );
...
export default function VendorDashboard() {
  ...
  const [stats, setStats] = useState<DashboardStats>(() => computeStats([], 0));
  ...
  const loadDashboard = async () => {
    ...
    setError(null);
    try {
      const [profileResponse, reservationsResponse, stallsResponse] = await Promise.all([
        userAPI.getProfile(),
        reservationsAPI.getMine(),
        stallsAPI.getAll(),
      ]);

      const stallsList = stallsResponse.stalls || [];
      const available = stallsList.filter((stall: Stall) => stall.isAvailable).length;
      const reservations = sortReservations(reservationsResponse.reservations || []);

      setProfile(profileResponse);
      setAvailableStalls(available);
      setRecentReservations(reservations);
      setStats(computeStats(reservations, available));
    } catch (err: any) {
  ...
  const updateGenres = async () => {
    ...
    setGenreError(null);
    try {
      const response = await reservationsAPI.updateGenres(selectedReservation.id, genres);
      const updatedReservation: Reservation = {
        ...selectedReservation,
        ...response.reservation,
        stall: selectedReservation.stall,
      };

      setRecentReservations((prev) => {
        const next = prev.map((reservation) => (reservation.id === updatedReservation.id ? updatedReservation : reservation));
        setStats(computeStats(next, availableStalls));
        return next;
      });
      setShowGenreModal(false);
      setSelectedReservation(null);
    } catch (err: any) {
      setGenreError(err?.message || "Failed to save genres.");
    } finally {
  ...
  const greeting = useMemo(() => {
    ...
    const contactName = profile.contactPerson?.split(" ")[0];
    return `${profile.businessName} - Welcome back${contactName ? `, ${contactName}` : ""}!`;
  }, [profile]);
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3 text-gray-600">
          <div className="w-12 h-12 border-4 border-[#2ab7c9] border-t-transparent rounded-full animate-spin" />
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-[#4598db] to-[#2ab7c9] rounded-2xl shadow-xl p-8 text-white">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              ...
              <div className="bg-white/30 p-4 border rounded-lg backdrop-blur-sm border-white/30">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/80 text-sm">Active Stalls</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/30 p-4 border rounded-lg backdrop-blur-sm border-white/30">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/80 text-sm">Ready Stalls</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/30 p-4 border rounded-lg backdrop-blur-sm border-white/30">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/80 text-sm">Pending Setup</p>
                  </div>
                </div>
              </div>
          ...
          <button
            onClick={loadDashboard}
            className="self-start inline-flex items-center gap-2 bg-white/20 border border-white/40 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-white/30 transition-colors"
          >
            <RefreshCw className={`w-4 h-4 ${refreshing ? "animate-spin" : ""}`} />
            Refresh Data
          </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {recentReservations.map((reservation) => (
              <div key={reservation.id} className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg">
                ...
                <div className="flex-1">
                  <p className="font-medium text-gray-900">Stall {reservation.stall?.name ?? reservation.stallId}</p>
                          ...
                          <div className="flex items-start gap-4 mb-4">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-600">
                                <div>
                                  <span className="font-medium">Location:</span> {reservation.stall?.location || "TBD"}
                                </div>
          ...
          <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl shadow-sm p-6 text-white">
            ...
            <div className="mt-4 pt-4 border-t border-white/20">
              <p className="text-xs text-white/80 leading-relaxed">
                The largest book fair and exhibition in Sri Lanka. Showcase your publications to thousands of visitors
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold">
                  {stats.totalReservations}/{MAX_STALLS}
                </span>
              </div>
              <div className="flex justify-between items-center">
      ...
      {showGenreModal && selectedReservation && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <h3 className="text-xl font-bold text-gray-900 mb-2">Literary Genres for Stall {selectedReservation.stall?.name ?? selectedReservation.stallId}</h3>
            <p className="text-gray-600 text-sm mb-6">
              Add the literary genres you'll be displaying at your stall. This helps visitors find your publications.
            </p>

            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 mb-2">Add New Genre</label>
              <div className="flex gap-2">
                ...
                <button
                  onClick={addGenre}
                  className="px-4 py-2 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg font-semibold hover:bg-blue-600 hover:text-white transition-all shadow-md"
                >
                  Add
                </button>
              </div>
              {genreError && <p className="mt-2 text-sm text-red-600">{genreError}</p>}
            </div>
              ...
              {genres.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {genres.map((genre) => (
                    ...
                    >
                      {genre}
                      <button onClick={() => removeGenre(genre)} className="text-white hover:text-gray-200 ml-1">
            ...
            <div className="flex gap-3 pt-4 border-t border-gray-200">
              ...
              <button
                ...
                className="flex-1 bg-gradient-to-r from-blue-600 to-blue-700 text-white py-2 rounded-lg font-medium hover:shadow-lg transition-all disabled:opacity-60"
              >
